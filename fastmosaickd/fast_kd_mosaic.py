...
parser.add_argument('--warmup', default=0, type=int, metavar='N', help='which epoch to start kd')
...
parser.add_argument('--reset_l0', default=0, type=int, help='reset l0 in the generator during training')
parser.add_argument('--reset_bn', default=0, type=int, help='reset bn layers during training')
parser.add_argument('--bn_mmt', default=0, type=float, help='momentum when fitting batchnorm statistics')
parser.add_argument('--is_maml', default=1, type=int, help='meta gradient: is maml or reptile')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    args.logger = engine.utils.logger.get_logger(log_name, output='checkpoints/MosaicKD/log-%s-%s-%s-%s%s.txt' % (
    ...
    teacher_1 = teacher_1.cuda(args.gpu)
    teacher_2 = teacher_2.cuda(args.gpu)
    teacher_3 = teacher_3.cuda(args.gpu)
    teacher_4 = teacher_4.cuda(args.gpu)
    teacher_5 = teacher_5.cuda(args.gpu)
    teacher = registry.EnsembleModel([teacher_1, teacher_2, teacher_3, teacher_4, teacher_5])           # setup ensemble teacher
    ...
    # Setup synthesizer
    ############################################
    synthesizer = datafree.synthesis.FastMetaSynthesizer(teacher, student, netG, netD, ood_loader, nz=args.z_dim,
                                                         num_classes=num_classes, img_size=(3, 32, 32),
                                                         init_dataset=args.cmi_init, save_dir=args.save_dir, device=args.gpu,
                                                         ...
                                                         warmup=args.warmup, lr_g=args.lr_g, lr_z=args.lr_z, adv=args.adv,
                                                         bn=args.bn, oh=args.oh, reset_l0=args.reset_l0, reset_bn=args.reset_bn,
                                                         bn_mmt=args.bn_mmt, is_maml=args.is_maml, local=args.local, 
                                                         align=args.align, balance=args.balance)  # todo 参数缺失的设定
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # todo
        for _ in range(1):          # todo kd_steps设定
            vis_results, cost = synthesizer.synthesize()
            time_cost += cost
            if epoch >= args.warmup:            # todo args.warmup参数设定
                train(synthesizer, ood_iter, val_loader, [student, teacher], criterion, optim_s, epoch, args)            # todo train要改
...
def train(synthesizer, train_loader, val_loader, model, criterion, optimizer, epoch, args):
    global time_cost
    global best_acc1
    student, teacher = model
    optim_s = optimizer
    ood_iter = train_loader
    student.train()
    teacher.eval()                              # todo ensemble eval的方式

    for i in range(args.kd_steps):              # todo kd_steps? mosaic_kd=5
        images = synthesizer.sample()
        ood_images = ood_iter.next()[0]
        if args.gpu is not None:
            images = images.cuda(args.gpu, non_blocking=True)
            ood_images = images.cuda(args.gpu)
        with args.autocast():
            with torch.no_grad():
                images = args.normalizer(images)
                images = torch.cat([images, ood_images])
                t_out = teacher(images)           # todo return_features
        ...
        if args.print_freq > 0 and i % args.print_freq == 0:
            acc1 = validate(optim_s.param_groups[0]['lr'], val_loader, student, criterion, args)
            print(
                'Epoch={current_epoch} Iter={i}/{total_iters}, Acc={acc:.4f} loss_s={loss_s:.4f}, Lr={lr:.4f}'
                .format(current_epoch=epoch, i=i, total_iters=len(train_loader), acc=float(acc1), loss_s=loss_s.item(), lr=optim_s.param_groups[0]['lr']))
            student.train()
            is_best = acc1 > best_acc1
            best_acc1 = max(acc1, best_acc1)
